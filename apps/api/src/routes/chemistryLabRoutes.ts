...
import { submitChemAttempt, regenerateChemAttemptPdf } from '../services/chemistryLabService';
...
// Chem Lab is open to ALL roles -- no requireRole gate, same as Tech Lab.
// No aiLimiter here either: unlike every other generator route, this one
// never calls an AI provider (see chemistryLabService.ts's header comment).
...
router.post('/attempts', validate(submitSchema), async (req: AuthenticatedRequest, res, next) => {
  try {
    const result = await submitChemAttempt(req.body, req.user!.id, req.supabase);
...
router.delete('/attempts/:id', async (req: AuthenticatedRequest, res, next) => {
  try {
    ...
    if (error) throw error;
    res.json({ success: true, message: 'Chem Lab attempt deleted' });
  } catch (error) {
    next(error);
  }
});
...
router.post('/attempts/:id/regenerate-pdf', async (req: AuthenticatedRequest, res, next) => {
  try {
    const result = await regenerateChemAttemptPdf(req.params.id, req.user!.id, req.supabase);
...
router.get('/attempts/:id/pdf', async (req: AuthenticatedRequest, res, next) => {
  try {
    const path = `chem-lab-reports/${req.user!.id}/${req.params.id}.pdf`;
