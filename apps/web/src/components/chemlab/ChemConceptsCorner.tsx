    ...
    <div className="grid grid-cols-1 md:grid-cols-[240px_1fr] gap-4">
      ...
      <div className="glass-card rounded-3xl p-5 md:p-7 space-y-5">
        <div className="flex items-start justify-between gap-3 flex-wrap">
          ...
          <SpeakButton
            label="Listen to All"
            text={`${active.name}. ${active.statement} ${active.whyItMatters} Here's why. ${active.explanationSteps.join(' ')} ${active.realLifeNote}`}
          />
        </div>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h4 className="font-bold text-sm">Why is it true? Step {stepIndex + 1} of {active.explanationSteps.length}</h4>
            <div className="flex items-center gap-1.5">
              ...
              <button
                onClick={() => setStepIndex((s) => Math.min(active.explanationSteps.length - 1, s + 1))}
                disabled={stepIndex === active.explanationSteps.length - 1}
                className="p-1.5 rounded-lg border-2 border-slate-200 dark:border-slate-800 disabled:opacity-30 hover:border-primary-400"
              >
          ...
          <div className="flex gap-1.5">
            {active.explanationSteps.map((_, i) => (
