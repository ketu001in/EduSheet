...
import {
  ...
  Loader2, CheckCircle2, XCircle, Printer, AlertTriangle, Microscope, TestTube, Dna, Leaf as LeafIcon,
} from 'lucide-react';
...
const BRANCH_ICON: Record<BiologyBranch, typeof Microscope> = {
  'human-biology': TestTube,
...
const BRANCH_LABEL: Record<BiologyBranch, string> = {
...
export default function NewBiologyExperimentPage() {
  const [phase, setPhase] = useState<Phase>('pick');
  const [experiment, setExperiment] = useState<BiologyExperiment | null>(null);
  ...
  const [selectedBranch, setSelectedBranch] = useState<BiologyBranch | null>(null);
  ...
  const gradeBand = selectedGradeNumber != null ? biologyGradeBandForClass(selectedGradeNumber) : null;
  const experimentsForBand = gradeBand ? BIOLOGY_EXPERIMENTS.filter((e) => e.gradeBand === gradeBand.id) : [];
  ...
  const pickExperiment = (exp: BiologyExperiment) => {
    setExperiment(exp);
  ...
  const handleSubmit = async () => {
    ...
    setSubmitError(null);
    try {
      const res = await submitBiologyAttempt({
    ...
    <div className={`${phase === 'pick' ? 'max-w-5xl' : 'max-w-3xl'} mx-auto pb-16 space-y-6`}>
      ...
      {phase === 'pick' && (
        <div className="space-y-6 select-none">
          <div>
            <h1 className="font-display text-3xl font-semibold mb-2 flex items-center gap-2"><Sparkles className="w-7 h-7 text-primary-600" /> New Biology Experiment</h1>
      ...
      {experiment && phase === 'done' && (
        <div className="glass-card rounded-3xl p-8 text-center space-y-4">
          ...
          <p className="text-sm text-slate-500">Your {experiment.title} attempt is saved to Biology Lab.</p>
          <div className="flex flex-wrap justify-center gap-3">
            {attemptId && (
              <button
                onClick={() => downloadBiologyAttemptPdf(attemptId)}
            ...
            <Link href="/biology-lab" className="btn-brutal px-5 py-2.5 border-2 border-slate-200 dark:border-slate-800 font-bold rounded-xl text-sm">
              Back to Biology Lab
            </Link>
          </div>
        </div>
